...
|---|---|---|---|
| **OI-1** | **REOPENED** | **At-rest store backend.** Operator ruled libSQL (NEW-3), and a complete `Store` impl exists in the standalone `secrets-store-libsql` crate (compiles, 9 offline tests). BUT the C-purity gate is honestly split (see that crate's `README.md`): the LITERAL grep (`libsql-ffi`/`libsql-sys`/`sqlite3-sys`) PASSES (the C-SQLite `core` is not pulled by `remote`), yet the SPIRIT fails — `remote → hrana → libsql-sqlite3-parser`, whose `build.rs` compiles `lemon.c` via `cc`, so a **C build-toolchain is still required**. Fallbacks are dead ends in 2026 (`libsql-client`/`libsql-hrana` 0.1 bitrotted; `libsql-hrana` alone is wire-types-only). **Crucial nuance:** the engine ALREADY needs `cc` at build time (blake3 SIMD asm), so the real tenet is **"no C *library* in the trust boundary"** (no SQLite/OpenSSL/aws-lc — still UPHELD), not "no `cc` ever." Under that precise tenet libSQL `remote` is adoptable (no runtime C lib; `lemon.c` is build-time codegen); the strict C-toolchain-free reading needs blake3 `pure` + a pure-Rust Hrana client. The crate is held OUT of `[workspace.members]`; the workspace stays green on `inmem-store`. | REOPENED — operator decision: (a) accept a C build-toolchain (blake3+lemon.c) and adopt the crate, or (b) require strict C-toolchain-free (blake3 `pure` + build/await a pure-Rust Hrana client). |
