...
|---|---|---|---|
| **OI-1** | RESOLVED (NEW-3) | **At-rest store backend = libSQL.** Operator ruled (NEW-3, see `SERVER-MODE.md`): libSQL is required because its server/replica/sync serves remote thin clients — a pure-Rust local-only store (redb) cannot. The bundled C SQLite is an ACCEPTED, SCOPED waiver: ALL libSQL is quarantined in a NEW `secrets-store-libsql` crate (consumed ONLY by secretd, behind the `Store` trait); the engine LIB stays pure-Rust and the no-C gate is now per-crate (engine green). The C core is ISOLATED (recommended: embedded sqld on loopback + secretd's pure-Rust `remote` client; in-process embedded only with a recorded risk acceptance). App-AEAD stays authoritative; `db/schema.sql` remains canonical. | RESOLVED; Phase 1 targets the libSQL `Store` impl in the store crate. |
